...
async function callGemini(key: string, system: string, messages: any[]): Promise<string> {
  ...
  // ═══ قياس ٨ سبتمبر ٢٠٢٦: الترتيب نفسه هو سبب البطء ═══
  // ثلاث عشرة مناداةً حيّةً للدالّة أعطت زمنًا ثنائيّ النمط:
  //   ١٫٧–٢٫٩ث في أربعٍ منها  (النموذج الأوّل ردّ فورًا)
  //   ٨–١٨٫٤ث في سبعٍ         (الأوّل سقط بعد ~١٠ث ثمّ ردّ الثاني)
  //   ٣٥٫٨ و٣٧٫٠ث في اثنتين   (تجاوزتا مهلة العميل ٢٨ث فرأى الطفل ردًّا آليًّا)
  // فالوسيط نحو ١٣ث، وطفلٌ في التاسعة ينتظر ثلاث عشرة ثانيةً أمام
  // «🤔 يفكّر بالرد...» — وهو بعينه بلاغ الأب الأقدم: «يتأخّر الردّ مرّة،
  // سبّب لهم ملل».
  // والعلّة أنّ المزدحم مُقدَّمٌ في الطابور: كلّ سقطةٍ له تكلّف نحو عشر
  // ثوانٍ تُدفع من صبر الطفل قبل أن يبدأ الثاني أصلًا. فقُدّم المستقرّ،
  // وبقي الأحدث ثانيًا — فمن يربح يربح سريعًا، ومن يخسر لا يُؤخّر غيره.
