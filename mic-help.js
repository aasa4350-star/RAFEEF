...
(function(g){
  ...
  /* ===== إذن المايكروفون قبل بدء التعرّف =====
     ...
     ═══ وقد نُقض هذا كلّه بالدليل (٦ سبتمبر ٢٠٢٦) ═══
     جرّبنا يومها إمساك المسار وتسليمه لأزور مرّةً واحدة، فانقلب الأمر
     إلى ما هو أسوأ: انقطعت المحادثة من دورها الثاني عند الأربعة جميعًا.
     وبيانات الجدول قاطعة — قبل ٥ سبتمبر ٣٠ جلسةً في ٦٤ دورًا بصفر
     إخفاق، وبعده كلّ جلسةٍ دورٌ واحدٌ بالضبط وعشرات الإخفاقات.
     والسبب أنّ المسار المستعمَل في سياقٍ صوتيٍّ أُغلق لا يعود يُعطي
     صوتًا في سياقٍ جديد على سفاري، والمكتبة تُغلق السياق بعد كلّ تعرّف.

     فرجعنا إلى مسارٍ جديدٍ لكلّ تعرّف (fromDefaultMicrophoneInput)، وهو
     ما كان يعمل أربعة أيّامٍ بلا إخفاق. والدوالّ أدناه (stream/release/
     releaseSoon) بقيت لأنّ الصفحات تستدعيها، ولم تعد تُمسك شيئًا.
     وتبقى فتحةُ الإذن في ensure ثمّ تُحرَّر فورًا — وهي حال ٢ سبتمبر
     التي عملت فيها المحادثة بصفر إخفاق. */
  ...
  /* ═══ لكلّ تعرّفٍ مسارُ مايكٍ جديد — لا مسارٌ مُعادُ استعماله ═══
     بلاغ الأب (٦ سبتمبر ٢٠٢٦): «المحادثات ما تضبط معهم». والبيانات
     دلّت على اليوم بعينه:

       ٠١–٠٤ سبتمبر: ٣٠ جلسةً · ٦٤ دورًا · صفر إخفاق
       ٠٥ سبتمبر:     ٦ جلسات · ٦ أدوار  · ٧ إخفاقات
       ٠٦ سبتمبر:     جلسة    · دورٌ واحد · ١١ إخفاقًا

     والعمود الحاسم «الأدوار»: قبل الخامس كانت الأدوار أكثر من الجلسات
     (حوارٌ متّصل)، ومنذ الخامس صارت كلّ جلسةٍ دورًا واحدًا بالضبط —
     أي أنّ الدور الأوّل ينجح والثاني يموت بلا ردٍّ حتى تنقضي المهلة.
     وذلك يوم شحنتُ «فتح المايك مرّةً واحدة» وتسليمَ مسارٍ مُمسَك.

     والسبب معروفٌ في سفاري: المسار (MediaStream) إذا استُعمل في
     MediaStreamAudioSourceNode ثمّ أُغلق سياقُه الصوتيّ، لم يعد يُعطي
     صوتًا في سياقٍ جديد — فيسمع أزور صمتًا فلا يردّ. والمكتبة تُغلق
     السياق بعد كلّ تعرّف، فيصحّ الدور الأوّل ويصمت ما بعده.

     فرجعنا إلى ما كان يعمل أربعة أيّامٍ بصفر إخفاق: مسارٌ جديد لكلّ
     تعرّفٍ عبر fromDefaultMicrophoneInput. وإمساكُ المسار لا يُوفّر
     شيئًا يُذكر مقابل أن يقطع الحوار من دوره الثاني. */
  function audioConfig(SDK){
    return SDK.AudioConfig.fromDefaultMicrophoneInput();
  }

  function ensure(statusEl, onReady, onDenied){
    /* الإذنُ يُطلب مرّةً واحدة في عمر الصفحة، ثمّ نمضي فورًا. وهذا ليس
       تحسينًا للسرعة: طلبُ getUserMedia قبل كلّ دورٍ يفتح مسارًا ثمّ
       يغلقه في اللحظة التي تفتح فيها أزور مسارها — وهي المزاحمة نفسها
       التي وصفناها. فتحةٌ واحدةٌ للإذن، وما بعدها لأزور وحدها. */
    ...
    navigator.mediaDevices.getUserMedia({ audio:true }).then(function(st){
      /* نُحرّر مسار الإذن فورًا: أزور تفتح مسارها لكلّ تعرّف، وإمساكُ
         مسارنا لا يُفيدها ويُبقي مؤشّر التسجيل مضاءً بلا سبب. */
      try{ st.getTracks().forEach(function(t){ t.stop(); }); }catch(e){}
      g.__micGranted = true;
